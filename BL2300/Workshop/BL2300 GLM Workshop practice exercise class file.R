...
model3<-glm(fleas~ncats + trtmt + carpet, data=cat) ##No interaction
par(mfrow=c(2,2))
plot(model2)
plot(model3)
AIC(model2, model3)
...
par(mfrow=c(1,1))
plot(fleas~ncats, data=cat, type="n", ylab="Log Number of Fleas", xlab="Number of Cats")
...
fit3
 ###work out predicted values
fit4 <- data.frame(fit3, predict(model3, fit3, se.fit = T)) ##NOTE CHANGE model2 to your best model name
fit4
str(fit4)
